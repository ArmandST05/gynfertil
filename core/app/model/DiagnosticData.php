...
class DiagnosticData {
	...
	public static function getTotalByDiagnosticIdSexRange($diagnosticId,$sexId,$startDate,$endDate,$startAge,$endAge){
		...
		AND SUBSTRING(reservations.date_at,0,10) >= '$startDate'
		AND SUBSTRING(reservations.date_at,0,10) >= '$endDate'
		AND patients.birthday != null AND patients.birthday != '' 
		AND TIMESTAMPDIFF(YEAR, patients.birthday, SUBSTRING(reservations.date_at,0,10)) >= '$startAge' 
		AND TIMESTAMPDIFF(YEAR, patients.birthday, SUBSTRING(reservations.date_at,0,10)) <= '$endAge'";
	...
	public static function getTotalByDiagnosticIdSexNonAge($diagnosticId,$sexId,$startDate,$endDate){
		...
		AND SUBSTRING(reservations.date_at,0,10) >= '$startDate'
		AND SUBSTRING(reservations.date_at,0,10) >= '$endDate'
		AND (patients.birthday = null OR patients.birthday = '')";

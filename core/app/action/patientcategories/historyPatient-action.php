...
$sql = "SELECT id, name,relative_name, calle, tel, email, ref,status FROM pacient";
...
if( !empty($requestData['search']['value']) ) {
	...
	$sql = "SELECT pacient.id, pacient.name,relative_name, calle, tel, email, ref,status,";
	...
	$sql.= " FROM pacient";
	$sql.= " LEFT JOIN patient_category_treatments ON pacient.id = patient_category_treatments.patient_id";
	...
	$sql.= " WHERE pacient.name LIKE '%".$requestData['search']['value']."%' ";    // $requestData['search']['value'] contains search parameter
	...
	$sql.= " LEFT JOIN patient_treatments ON patient_category_treatments.patient_treatment_id = patient_treatments.id";
	$sql.= " GROUP BY pacient.id";
	$sql.= " ORDER BY pacient.id DESC LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
	$query = mysqli_query($conn, $sql) or die("./?action=pacients: get PO");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["category_id"] && $row["category_id"] == 3){
		$nestedData[] = "<td><b>".$row["category_name"]." ".$row["treatment_name"]."</b></td>";
	}
	else if($row["category_id"]){
		$nestedData[] = "<td><b>".$row["category_name"]."</b></td>";
	}
	else{
	...
	$categories_history = "";
	$categories = PatientCategoryData::getAllPatientCategories($row["id"]);
	$total_categories = count($categories);

	foreach($categories as $index => $category){
		if($index == 0) $categories_history .= "<b>".$category->category_abbreviation."</b>";
		else $categories_history .= $category->category_abbreviation;

		if(($index+1) != $total_categories){
			$categories_history .=", ";
		}
	}

	$nestedData[] = "<td>".$categories_history."</td>";

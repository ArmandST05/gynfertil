...
$searchPaymentTypeId = $_POST['searchPaymentTypeId'];
...
$actualDateTime = date("Y-m-d")." 00:00:00";
// getting total number records without any search
$sql = "SELECT o.status_id,o.description,o.bank,o.is_invoice,o.invoice_number,o.id,o.total,o.created_at,p.`name`, CONCAT(ELT(WEEKDAY(o.created_at) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS day_name, DATE_FORMAT(o.created_at,'%d/%m/%Y') as date 
            FROM operations o, patients p 
            WHERE p.id = o.patient_id 
            AND o.operation_type_id='2' 
            AND operation_category_id='1' 
            AND o.branch_office_id = '$searchBranchOfficeId' ";
        if($userType == "r" || $userType == "co"){
            $sql .= " AND (o.created_at >= '" . $actualDateTime . "' OR o.status_id = 0) ";
        }
        if($searchPaymentTypeId != "all"){
            $sql .= " AND (SELECT op.id FROM operation_payments op WHERE op.operation_id = o.id AND op.payment_type_id = '" . $searchPaymentTypeId . "' LIMIT 1) IS NOT NULL";
        }
        if($searchStatusId != "all"){
            $sql .= " AND o.status_id = '" . $searchStatusId . "' ";
        }
...
if (!empty($requestData['search']['value'])) {
    ...
    $sql .= " WHERE  p.id=o.patient_id  AND operation_type_id='2' AND operation_category_id='1' AND o.branch_office_id = '$searchBranchOfficeId' ";
    if($userType == "r" || $userType == "co"){
        $sql .= " AND (o.created_at >= '" . $actualDateTime . "' OR o.status_id = 0) ";
    }
    if($searchPaymentTypeId != "all"){
...
while ($row = mysqli_fetch_array($query)) {  // preparing an array
    ...
    if ($userType == "su") {
        $nestedData[] =  '<td >
            <!--<a href="reports/onesale-word.php&id=' . $row["id"] . '" class="btn btn-xs btn-success"><i class="fas fa-print"></i></a>-->
        </td>';
        $nestedData[] = '<td >

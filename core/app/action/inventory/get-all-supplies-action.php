...
$sql = "SELECT id, name, type,inventary_min  FROM product WHERE type='INSUMOS'";

$query=mysqli_query($conn, $sql) or die("./?action=inventory/get-all-supplies: get InventoryItems");
...
if( !empty($requestData['search']['value']) ) {
	...
	$sql = "SELECT  id, name, type,inventary_min ";
	$sql.=" FROM product";
	...
	$query=mysqli_query($conn, $sql) or die("./?action=inventory/get-all-supplies: get PO");
	...
	$query=mysqli_query($conn, $sql) or die("./?action=inventory/get-all-supplies: get PO"); // again run query with limit
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = '<p style="background-color:' . $color . '" class="success"> '.$row["name"];
	$nestedData[] = '<p style="background-color:' . $color . '" class="success"> '.$q;
   	$nestedData[] = '<p style="background-color:' . $color . '" class="success"> '.$row["type"];

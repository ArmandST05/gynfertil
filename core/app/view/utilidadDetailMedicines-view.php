...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<h1>Detalle Ingresos Medicamentos Reporte de Utilidad</h1>
		</div>
	</div>
	<br>
	<!--- -->
	<div class="row">
		<div class="col-md-12">
			...
			$sellDetails = SellData::getDetSellMedicineId($productId, $f1, $f2);
			...
			<table class="table table-bordered table-hover" style="width:750px;" id='datosexcel' border='1'>
				...
				foreach ($sellDetails as $detail) {
					...
					echo "
						<tr class='success'>
						<td>$product->name</td>
						<td>".date_format(date_create($detail->created_at),'d-m-Y H:i')."</td>
						<td>$patient->name</td>
						<td>$detail->q</td>
						<td>$$detail->price</td>
				...
				echo "<tr><td></td><td></td><td><label>Total:</label></td><td><label>" .$totalQuantity . "</label></td><td></td><td class='success'><label>$" . number_format($totalSells, 2) . "</label></td></tr>";
				?>
			</table>
		</div>
	</div>

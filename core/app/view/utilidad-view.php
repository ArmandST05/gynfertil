...
<section class="content">
	...
	<div class="row">
		<div class="col-md-12">
			...
			$ConIng = SellData::getAllSellDateUt($f1, $f2);
			$ConEgre = SellData::getAllBuyDateUt($f1, $f2);
			?>
			...
			<table class="table table-bordered table-hover" style="width:750px;" id='datosexcel' border='1'>
				...
				foreach ($ConIng as $con) {
					...
					$res = SellData::getAllSellDateRUt($con->product_id, $f1, $f2);
			...
			<td>";
						foreach ($res as $key1) {
							...
							$medic_name = (isset($med)) ? $med->name:'Venta Libre';
							echo "<a href='./index.php?view=utilidadDetailMedic&medicId=$key1->idMedic&productId=$con->product_id&sd=$f1&ed=$f2'>$medic_name </a><label>Can: </label> $key1->q <label>Total: </label> " . number_format($key1->total, 2) . "<br>";
						}

						echo "</td></tr>";
					}
				}
				/****MOSTRAR INGRESOS MEDICAMENTOS*********/
				$Cat = SellData::getNameCat(8);
				$cM = 0;
				$tM = 0;
				foreach ($Cat as $c) {
					$det = SellData::getdetMed($f1, $f2);
					foreach ($det as $k) {
						$cM += $k->q;
						$tM += $k->total;
					}
			...
			<td>";
					foreach ($det as $k) {
						...
						echo "<a href='./index.php?view=utilidadDetailMedicines&productId=$k->product_id&sd=$f1&ed=$f2'>$pro->name </a><label>Can: </label> $k->q <label>Total: </label> " . number_format($k->total, 2) . "<br>";
					}
					echo "</td></tr>";
				}
			...
			<table class="table table-bordered" style="width:350px;" id='datosexcel'>
				...
				foreach ($typ as $t) {
					$ing = SellData::getIngresosUt($t->id, $f1, $f2);
			...
			<table class="table table-bordered" style="width:350px;" id='datosexcel' border='1'>
				...
				foreach ($typE as $t) {
					$eg = SellData::getEgresosEUt($t->id, $f1, $f2);

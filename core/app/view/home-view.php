...
$fecha1 = date('Y') . "-" . date('m', strtotime('-1 month')) . "-" . date('01');

if (date('m') == 01) {
  $fecha1 = date('Y', strtotime('-1 year')) . "-" . date('m', strtotime('-1 month')) . "-" . date('01');
} else {
  $fecha1 = date('Y') . "-" . date('m', strtotime('-1 month')) . "-" . date('01');
}
...
if ($tipo2 == "do") {
  ...
  if ($medicData->category_id == 8) {
    /*Embriólogos(8) visualizan las categorías de clave(),cita(id_col):
    Cita(Inseminación(6),Biologo(7), Embriólogo(10))
    */
    $sql = "select  r.clave,r.id,r.pacient_id,r.medic_id,r.date_at,r.time_at_final,date_at_final,DATE_FORMAT(r.time_at,'%h:%i %p') time_at,
      ...
      AND date_at >='$fecha1' order by date_at ASC";
  } else {
    $sql = "select  r.clave,r.id,r.pacient_id,r.medic_id,r.date_at,r.time_at_final,date_at_final,DATE_FORMAT(r.time_at,'%h:%i %p') time_at,
      r.user_id,r.note,r.status_reser,r.color,r.color_letra,negritas, 
      TRIM(p.name)patient_name,TRIM(p.tel) patient_tel
  ...
  $sql = "select r.clave,r.id,r.pacient_id,r.medic_id,r.date_at,date_at_final,r.time_at_final,DATE_FORMAT(r.time_at,'%h:%i %p') time_at,
    ...
    FROM reservation r
    LEFT JOIN pacient p ON r.pacient_id = p.id
    WHERE date_at >='$fecha1' order by date_at ASC";
}
$req = $bdd->prepare($sql);
$req->execute();
$events = $req->fetchAll();
      ...
      $('#calendar').fullCalendar({
        ...
        eventRender: function(event, element) {
          element.bind('click', function() {
            ...
            var url = "./?view=editreservation&id=" + id + "&id_paciente=" + id_pac + "&fecha=" + fecha + "";

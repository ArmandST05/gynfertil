...
try {
    ...
    //OBTENER DATOS DEL FORMATO/TABLA SECCIONES Y CONTENIDO DE TABLA DE ÓVULOS FIVTE
    ...
    foreach($ovuleImagesData as $image){
        $imagesData[$image->procedure_section_id][] = $image;
    }
    $arrayOvuleImagesFivte = array_chunk($imagesData[1], 4);
    $arrayOvuleImagesRec = array_chunk($imagesData[2], 4);
    ...
    <table width="100%">
        <tr>
            ...
            <td class="title-document">CICLO MIXTO DONANTE PACIENTE <br>' . $embryologyProcedure->treatment_code . '</td>
        </tr>
    </table>
    ...
    <table width="100%">
        <tr>
            <td class="subtitle-document bold">FIVTE DE LA PACIENTE</td>
        </tr>
    </table>
    <table width="100%" class="patient-details">
    <tr class="title">
    ...
    <td>' . $embryologyProcedure->getDateMonthFormat($procedureDetails['148']) . '</td>
    <td>' . $procedureDetails['149'] . '</td>
    <td>' . $procedureDetails['164'] . '</td>
</tr>
<tr class="title">
    <th>Beta Hcg</th>
...
<tr>
<td>' . $procedureDetails['150'] . '</td>';
    $femaleConsent = (($procedureDetails['153'] == 1) ? "Sí" : "No");
    $maleConsent = (($procedureDetails['154'] == 1) ? "Sí" : "No");
...
<tr>
<td>' . $procedureDetails['165'] . '</td>
<td>' . $procedureDetails['155'] . '</td>
<td>' . $procedureDetails['156'] . '</td>
<td>' . $procedureDetails['157'] . '</td>
<td>' . $procedureDetails['158'] . '</td>
</tr>
<tr class="title">
    <th>Degenerado</th>
...
<tr>
<td>' . $procedureDetails['159'] . '</td>
<td>' . $procedureDetails['160'] . '</td>
<td>' . $procedureDetails['161'] . '</td>
</tr>
</table>
    <br>
    ...
    foreach ($sections as $section) {
        $html .= '<th colspan="' . $section->total_section_details . '">' . $section->name . '<br>' . calculateSectionDate($section->day_number, 148) . '</th>';
    }
  ...
  <table width="100%">
  <tr>
      <td class="subtitle-document bold">REC-DONOVO</td>
  </tr>
</table>
  <table width="100%" class="patient-details">
    <tr class="title">
        <th>Ciclo</th>
        <th>Fecha de aspiración folicular</th>
        <th colspan="2">Aspiración - Estradiol</th>
        <th>Consentimiento femenino</th>
    </tr>';
    $femaleConsent = (($procedureDetails['170'] == 1) ? "Sí" : "No");
    $maleConsent = (($procedureDetails['171'] == 1) ? "Sí" : "No");
    $html .= '
    <tr>
    <td>' . $actualCycle . '</td>
    <td>' . $embryologyProcedure->getDateMonthFormat($procedureDetails['166']) . '</td>
    <td colspan="2">' . $procedureDetails['167'] . '</td>
    <td>' . $femaleConsent . '</td>
</tr>
<tr class="title">
    <th colspan="2">Diagnóstico</th>
    <th>Código ciclo de donación de óvulos</th>
    <th>Código donadora de óvulos</th>
    <th>Consentimiento masculino</th>
</tr>
<tr>
<td>' . $treatmentDiagnostics . '</td>
<td>' . $procedureDetails['180'] . '</td>
<td>' . $procedureDetails['181'] . '</td>
<td>' . $maleConsent . '</td>
</tr>
<tr class="title">
<th colspan="5">Semen</th>
...
<tr>
<td>' . $procedureDetails['172'] . '</td>
<td>' . $procedureDetails['173'] . '</td>
<td>' . $procedureDetails['174'] . '</td>
<td>' . $procedureDetails['175'] . '</td>
<td>' . $procedureDetails['176'] . '</td>
</tr>
<tr>
    <th class="title">Fecundación-Ovocitos inseminados</th>
    <th class="title">Fecundación-Fertilizados</th>
</tr>
<tr>
<td>' . $procedureDetails['177'] . '</td>
<td>' . $procedureDetails['178'] . '</td>
</tr>
</table>
    <br>
    <table id="ovulesDataTable" cellspacing="0" class="ovule-details cell-border compact" style="width:100%">
    <thead>
      <tr class="table-title">
        ...
        <th rowspan="2">#</th>
    ';
    foreach ($sectionsRec as $section) {
        $html .= '<th colspan="' . $section->total_section_details . '">' . $section->name . '<br>' . calculateSectionDate($section->day_number, 166) . '</th>';
    }
    $html .= '<th colspan="3">DESTINO</th>
        <th rowspan="2">MUESTRA SEMEN</th>
      </tr>
      <tr  class="table-title">';
    foreach ($sectionDetailsRec as $sectionDetail) {
        $html .= '<th>' . $sectionDetail->name . '</th>';
    }
    $html .= '<th>TE</th>
        <th>C</th>
    ...
    <tbody>';
    foreach ($procedureOvulesRec as $ovule) {
        $ovuleSectionDetailValues = PatientOvuleData::getSectionDetailsByOvuleId($embryologyProcedure->patient_treatment_id, 2, $ovule->id);
        $html .= '<tr>
            <td>' . $ovule->getOriginPatientTreatment()->treatment_code . '</td>
            <td>' . $ovule->getDonor()->donor_id . "-" . $ovule->procedure_code . '</td>';
        foreach ($ovuleSectionDetailValues as $ovuleValue) {
            $html .= '<td>' . $ovuleValue->value . '</td>';
        }
        $html .= '<td>' . (($ovule->end_ovule_status_id == 3) ? "X" : "") . '</td>
...
<table width="100%">
<tr>
    <td class="subtitle-document bold">IMÁGENES FIVTE</td>
</tr>
</table>
<table class="patient-details">';
    foreach ($arrayOvuleImagesFivte as $ovuleImages) {
        $html .= '<tr>';
    ...
    <table width="100%">
        <tr>
            <td class="subtitle-document bold">IMÁGENES REC-DONOVO</td>
        </tr>
    </table>
    <table class="patient-details">';
        foreach ($arrayOvuleImagesRec as $ovuleImages) {
            $html .= '<tr>';
            foreach ($ovuleImages as $ovuleImage) {
                $html .= '<td width="250px"><img width="230px" height="230px" src="' . $ovuleImage->path . '" /></td>';
            }
            ...
            <tr class="title">';
            foreach ($ovuleImages as $ovuleImage) {
                $html .= '<th width="250px">#' . $ovuleImage->procedure_code. '</th>';
            }
            $html .= '</tr>';
        }
    $html .= '</table>';

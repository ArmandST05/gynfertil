...
try {
    ...
    <table width="100%">
        <tr>
            ...
            <td class="title-document">DONACIÓN DE EMBRIONES <br>' . $embryologyProcedure->treatment_code . '</td>
        </tr>
    </table>
    <br>
    <table width="100%">
    ...
    <tr class="title">
        <th>Ciclo</th>
        <th>Consentimiento de donación de embriones</th>
        <th>Consentimiento de recepción de embriones donados</th>
        <th>Diagnóstico</th>
        <th>Ciclo de procedencia de embriones</th>
    </tr>';
    $donorConsent = (($procedureDetails['77'] == 1) ? "Sí" : "No");
    $recipientConsent = (($procedureDetails['78'] == 1) ? "Sí" : "No");
    $html .= '
    <tr>
    <td>' . $actualCycle . '</td>
    <td>' . $donorConsent. '</td>
    <td>' . $recipientConsent . '</td>
    <td>' . $treatmentDiagnostics . '</td>
    <td>' . $procedureDetails['79'] . '</td>
</tr>
</table>
    <br>
    ...
    $html .= '<th rowspan="2">PROCEDIMIENTO DESTINO</th>
    ...
    foreach ($procedureOvules as $ovule) {
        ...
        <td>'.$ovule->donor_id . "-" . $ovule->procedure_code.'</td>';
        
        foreach ($ovuleSectionDetailValues as $ovuleValue) {
            $html .= '<td style="background-color:'.(($ovuleValue->ovule_section_detail_id == '102' && $ovuleValue->value != "") ? $ovuleValue->value : "#FFFFFF").'">
            ' . (($ovuleValue->ovule_section_detail_id != '102') ? $ovuleValue->value:"") . '</td>';
        }
        $html .= '<td></td>

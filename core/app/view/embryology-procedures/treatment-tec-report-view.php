...
try {
    ...
    <table width="100%">
        <tr>
            ...
            <td class="title-document">TRANSFERENCIA DE EMBRIONES CONGELADOS <br>' . $embryologyProcedure->treatment_code . '</td>
        </tr>
    </table>
    <br>
    <table width="100%">
    ...
    <tr class="title">
        ...
        <th>Beta Hcg</th>
        <th>Código de procedencia de embriones</th>
    </tr>
    <tr>
    <td>' . $actualCycle . '</td>
    <td>' . $embryologyProcedure->getDateMonthFormat($procedureDetails['50']) . '</td>
    <td>' . $procedureDetails['51'] . '</td>
    <td>' . $procedureDetails['52'] . '</td>
</tr>
</table>
    <br>
...
';
    ...
    foreach ($procedureOvules as $ovule) {
        ...
        foreach ($ovuleSectionDetailValues as $ovuleValue) {
            $html .= '<td style="background-color:' . (($ovuleValue->ovule_section_detail_id == "40" && $ovuleValue->value != "") ? $ovuleValue->value : "#FFFFFF") . '">
                    ' . (($ovuleValue->ovule_section_detail_id != '40') ? $ovuleValue->value : "") . '
                    </td>';
        }

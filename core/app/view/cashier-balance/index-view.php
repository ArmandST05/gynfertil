...
$sales = OperationData::getAllSelledProductsByBranchOfficeDate($searchBranchOfficeId, $startDate);
$expenses = OperationData::getAllExpensesByBranchOfficeDate($searchBranchOfficeId, $startDate);
?>
		...
		<div class="col-md-12">
			...
			<form>
				...
				<div class="row">
					...
					<?php if ($userType == "su") : ?>
						<div class="col-md-3">
							<input type="button" class="btn btn-primary btn-block" value="Exportar" id="btnExport" onclick="addLog(0,8,4,'Se descargó el archivo de Cortes del dia')">
						</div>
					<?php endif; ?>
				</div>
			</form>
		...
		<div class="col-md-12">
			...
			<table class="table table-bordered table-hover" id='datosexcel' style="width:750px;">
				...
				foreach ($sales as $sale) {
					...
					$detailsByMedic = OperationData::getAllMedicSalesByProductDate($searchBranchOfficeId, $sale->product_id, $startDate);
					...
					if ($productType <> 4) {
						...
						foreach ($detailsByMedic as $detail) {
							...
							echo "$medic->name <label>Cantidad: </label> $detail->quantity <label>Total: </label> " . number_format($detail->price, 2) . "<br>";
						}
						echo "</td></tr>";
					}
				}
			...
			<table class="table table-bordered" id='datosexcel' style="width:350px;">
				...
				foreach ($paymentTypes as $paymentType) {
					$inputs = OperationData::getInputsSales($searchBranchOfficeId, $paymentType->id, $startDate, $startDate);
					echo "
					<tr class='success'>
			...
			<table class="table table-bordered" id='datosexcel' style="width:350px;">
				...
				foreach ($paymentTypes as $paymentType) {
					$outputs = OperationData::getOutputsExpenses($searchBranchOfficeId, $paymentType->id, $startDate, $startDate);
					$subtotalOutput = 0;
					$subtotalOInvoice = 0;
